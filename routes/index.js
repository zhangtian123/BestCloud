...
router.get('/', function(req, res, next) {
  res.render('index', { title: 'BestCloud'});
});
...
router.get('/service', function(req, res, next) {
	...
	if(req.session.user)
	{
		...
		{
			res.render('personalProfileForManager',{ title: 'BestCloud',username:req.session.user.LoginCode,flag:req.session.user.FLAG,openid:req.session.user.WechatOpenID});
		}
		
		else if(req.session.user.UserFlag=="DC")
		{
			res.render('personalProfileForThird',{ title: 'BestCloud',username:req.session.user.LoginCode,flag:req.session.user.FLAG,openid:req.session.user.WechatOpenID});
		}
		else
		{
			res.render('personalProfileForClient',{ title: 'BestCloud',username:req.session.user.LoginCode,flag:req.session.user.FLAG,openid:req.session.user.WechatOpenID});
		}
	}
	else
	{
        ...
        if (!error && response.statusCode == 200) { 
        	if(body.indexOf('errcode')!=-1){
        		res.render('loginMain', { title: 'BestCloud'});
        	}
        	else{
...
router.get('/BoxAudit', function(req, res, next) {
	res.render('caseAuditing', { title: 'BestCloud' });
});

/* GET 接受打单. */
router.get('/CardAudit', function(req, res, next) {
	res.render('cardAuditing', { title: 'BestCloud' });
});
...
router.get('/registerMain', function(req, res, next) {
	res.render('registerMain', { title: 'BestCloud' });
});

/* GET 注册设置密码页. */
router.get('/registerSetPw', function(req, res, next) {
	res.render('registerSetPw', { title: 'BestCloud' });
});

/* GET 注册填写公司信息页. */
router.get('/registerCominfo', function(req, res, next) {
	res.render('registerCominfo', { title: 'BestCloud' });
});

/* GET 注册填写公司信息页. */
router.get('/takephoto', function(req, res, next) {
	res.render('takephoto', { title: 'BestCloud' });
});

/* GET 注册填写完成. */
router.get('/registerWait', function(req, res, next) {
	res.render('registerWait', { title: 'BestCloud' });
});

/* GET 注册审核. */
router.get('/registerAudit', function(req, res, next) {
	res.render('registerAuditing', { title: 'BestCloud' });
});

router.get('/registerDetail', function(req, res, next) {
	res.render('registerDetail', { title: 'BestCloud',UserName:req.session.register.registeruser,TaxNo:req.session.register.TaxNo,
	NameC:req.session.register.NameC,NameE:req.session.register.NameE,Address:req.session.register.Address,
	AddressEN:req.session.register.AddressEN,Email:req.session.register.Email,FilePath:req.session.register.FilePath });
});
...
router.get('/loginForgetPw', function(req, res, next) {
	res.render('loginForgetPw', { title: 'BestCloud' });
});

/* GET 重设密码. */
router.get('/loginSetPw', function(req, res, next) {
	res.render('loginSetPw', { title: 'BestCloud' });
});
...
router.get('/stateback', function(req, res, next) {
	res.render('boxtrack', { title: 'BestCloud' });
});

/* 预约提箱. */
router.get('/ordercase', function(req, res, next) {
	res.render('ordercase', { title: 'BestCloud' });
});

/* 预约打单. */
router.get('/ordercard', function(req, res, next) {
	res.render('ordercard', { title: 'BestCloud' });
});

/* GET 状态反馈之提箱. */
router.get('/statebackpickup', function(req, res, next) {
	res.render('statebackpickup', { title: 'BestCloud',XiangDanID:req.session.conInfo.XiangDanID,ContainerSize:req.session.conInfo.ContainerSize,ContainerType:req.session.conInfo.ContainerType,Remark:req.session.conInfo.Remark});
});

/* GET 状态反馈之送. */
router.get('/statebacksendbox', function(req, res, next) {
	res.render('statebacksendbox', { title: 'BestCloud',XiangDanID:req.session.conInfo.XiangDanID,ContainerSize:req.session.conInfo.ContainerSize,ContainerType:req.session.conInfo.ContainerType,Remark:req.session.conInfo.Remark,ContainerNo:req.session.conInfo.ContainerNo,SealNo:req.session.conInfo.SealNo});
});
...
router.get('/statebackrepay', function(req, res, next) {
	res.render('statebackrepay', { title: 'BestCloud',XiangDanID:req.session.conInfo.XiangDanID,ContainerSize:req.session.conInfo.ContainerSize,ContainerType:req.session.conInfo.ContainerType,Remark:req.session.conInfo.Remark,ContainerNo:req.session.conInfo.ContainerNo,SealNo:req.session.conInfo.SealNo});
});

/* GET 状态反馈之离厂. */
router.get('/statebackleave', function(req, res, next) {
	res.render('stackbackleave', { title: 'BestCloud',XiangDanID:req.session.conInfo.XiangDanID,ContainerSize:req.session.conInfo.ContainerSize,ContainerType:req.session.conInfo.ContainerType,Remark:req.session.conInfo.Remark,ContainerNo:req.session.conInfo.ContainerNo,SealNo:req.session.conInfo.SealNo});
});

/* GET 状态反馈之到厂. */
router.get('/statebackarrivefactory', function(req, res, next) {
	res.render('statebackarrivefactory', { title: 'BestCloud',XiangDanID:req.session.conInfo.XiangDanID,ContainerSize:req.session.conInfo.ContainerSize,ContainerType:req.session.conInfo.ContainerType,Remark:req.session.conInfo.Remark,ContainerNo:req.session.conInfo.ContainerNo,SealNo:req.session.conInfo.SealNo});
});

/* GET 状态反馈之进港. */
router.get('/statebackarriveport', function(req, res, next) {
	res.render('statebackarriveport', { title: 'BestCloud',XiangDanID:req.session.conInfo.XiangDanID,ContainerSize:req.session.conInfo.ContainerSize,ContainerType:req.session.conInfo.ContainerType,Remark:req.session.conInfo.Remark,ContainerNo:req.session.conInfo.ContainerNo,SealNo:req.session.conInfo.SealNo});
});
...
router.get('/boxsearch', function(req, res, next) {
	res.render('boxsearch', { title: 'BestCloud' });
});
...
router.post('/identifyCode',function(req,res,next){
	...
	sms.singleSmsSend(0,'86',emailaddress,num+'为您的注册验证码，请于5分钟内填写','','',function(data){
		...
		if(ret.result==0){
			...
			res.send({result:'验证码已发送成功，5分钟内有效'});
		}
		else{
			res.send({result:'验证码发送失败'});
		}
	});
...
router.post('/profile', upload.single('avatar'), function(req, res, next) {
	...
	var userstr = {UserID:guid1,CustomerID:guid2,Password:req.session.user2.PASSWORD,LoginCode:req.session.user2.USERCODE,EMail:req.session.user2.EMAIL,CREATEDATE:Time.getNowDate(0,0,0),RowState:"4"}
  ...
  var arrayphoto = [];
  var AccesList = {AccesID:guid3,FileName:filename,FilePath:filepath,RowState:"4"};
...
router.post('/statebackpickup', function(req, res, next) {
	...
	var orderNodesStr = {OrderXiangDanID:xiangDanID,IndexNo:indexNo,NodeCode:nodeCode,NodeName:nodeName,IsFinished:operateFlag,FinishTime:finishedTime,IsRefused:operateFlag?false:true,RefuseTime:operateFlag?null:finishedTime,CREATEBY:req.session.user.LoginCode,CREATEDATE:new Date().toLocaleString()};
...
router.post('/statebackfinish',function(req, res, next){
	...
	var indexNo = parameter.indexNo;
	var operateFlag = parameter.operateFlag;
  var orderNodesStr = {OrderXiangDanID:xiangDanID,IndexNo:indexNo,NodeCode:nodeCode,NodeName:nodeName,IsFinished:operateFlag,FinishTime:finishedTime,IsRefused:operateFlag=="true"?"false":"true",RefuseTime:operateFlag=="true"?null:finishedTime,CREATEBY:req.session.user.LoginCode,CREATEDATE:new Date().toLocaleString()};
  var orderNodes = JSON.stringify(orderNodesStr);
...
//		}
//	}
>>>>>>> 3f7ed4a4d6137707a594c6ff72415f38f44d3f1a
	console.log(predicate);
	console.log(values);
...
router.post('/bindingwx', function(req, res, next) {
	...
	Userwcf.ChangeOpenId(userId,openId,userFlag,function(result){
		...
		res.send({result:result,userID:userId,UserFlag:userFlag,WechatOpenID:openId});
	})
	}
});

router.get('/downloadapp', function(req, res, next) {
		res.render('downloadapp', { title: 'BestCLOUD'});
});
...
router.get('/DispatchList', function(req, res, next) {
	res.render('DispatchList', { title: 'BestCloud' });
});

router.get('/ConRecognition', function(req, res, next) {
	res.render('ConRecognition', { title: 'BestCloud' });
});
